...
showGraph:
    ...
    scaleTwo:
    mov dword[scale], SCALE2
    jmp scaleDone

    scaleThree:
    mov dword[scale], SCALE3
    jmp scaleDone
    ...
    mov rsi, 0
    clearNumTwo:
    mov byte[num2+rsi], 0x20
    inc rsi
    cmp rsi, 10
    jne clearNumTwo


    ;   Loop through each number and graph them
    mov r10, 0          ;   counter
    mov r9, 0           ;   temp number holder
    mov r8, "0"         ;   used to insert the index
    mov rdi, 0
    graphLp:

    mov byte[index2], r8b

    mov r9d, dword[r13+r10*4]
    mov dword[currCount], r9d

    ;   Insert the current count
    mov edi, dword[currCount]
    mov rsi, num2
    call int2octal

    ;   Calculate the number of stars
    mov eax, dword[currCount]
    cdq
    div dword[scale]
    cmp eax, 0
    je zeroStars
    ...
    ;   Clear the stars
    mov rsi, 0
    cmp dword[starCount], 0
    je noStars

    clearStar:
    mov byte[stars2+rsi], 0x20
    inc rsi
    cmp esi, dword[starCount]
    jne clearStar
